...
export const getOrderDetailsHelp = ({ needHelpLabel, helpLineNumberLabel, contactNumber, helpLineContactAriaLabel, telemetryContent }: IGetHelpInput): IHelp => {
    ...
    return ({
        ...
        needHelpLabel: <p className='ms-order-details__help-title'>
            {needHelpLabel}
        </p>,
        helpLineNumberLabel: <span className='ms-order-details__help-label'>
            {helpLineNumberLabel}
        </span>,
        contactNumber: (
            <a
                className='ms-order-details__help-content-number' aria-label={`${helpLineContactAriaLabel} ${contactNumber}`} href={`tel:${contactNumber}`}
                {...attributes}>
                {contactNumber}
            </a>
        ) }
