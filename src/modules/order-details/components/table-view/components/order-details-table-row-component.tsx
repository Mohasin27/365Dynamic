...
import {
    getDeliveryDescriptionByProductId, getDeliveryType, getDeliveryTypeString,
    IDeliveryTypeStringMap, ISalesStatusStringMap
...
export const OrderDetailsDesktopTableRowCellComponent: React.FC<IOrderDetailsTableRowProps> = (props: IOrderDetailsTableRowProps) => {
    const { data: { product, salesOrder, deliveryOptions } } = props;
    const salesLine = product.salesLine;

    const pickupMode = getDeliveryDescriptionByProductId(
        salesLine.ProductId, salesLine.DeliveryMode, deliveryOptions);
    ...
    const deliveryType = getDeliveryType(
        ...
        props.isRetailMultiplePickUpOptionEnabled, props.data.channelDeliveryOptionConfig, emailDeliveryModeCode);
        ...
        <>
            {
                props.isSelectionModeEnabled ? (
                    <td>
                        <OrderDetailsRowCheckboxComponent
                            {...props}
                        />
                    </td>
                ) : undefined
            ...
            <td>
                <OrderDetailsRowProductImageComponent
                    {...props}
                />
            </td>
            <td>
            ...
            <td>
                <div className={`${props.className}__product-info`}>
                    ...
                    <span className={`${props.className}__product-dimensions`}>
                        {
                            product.simpleProduct?.Dimensions?.map(
                                dimension => dimension.DimensionValue?.Value).filter(
                                dimensionValue => !StringExtensions.isNullOrWhitespace(dimensionValue)).join(
                                props.resources.orderDetailsProductDimensionsSeparator)
                        }
                    </span>
                    {
                        !product.isProductAvailable && <div className='msc-alert-danger'>
            ...
            {
                props.isCashAndCarryTransaction ? undefined : (
                    <td>
                        {salesStatusString}
                    </td>
                )
            ...
            <td>
                <BuySelectedComponent
                    ...
                    resources={{
                        ...
                        orderDetailsBuySelectedButtonAriaLabel: format(props.resources.orderDetailsBuyItAgainButtonAriaLabel, productName ?? '')
                    }}
 ...
 */
const OrderDetailsExtraActionsPopupComponent: React.FC<IOrderDetailsTableRowProps> = (props: IOrderDetailsTableRowProps): JSX.Element | null => {
    const { resources: { orderDetailsViewDetailsButtonAriaLabel, orderDetailsViewDetailsButtonText } } = props;
...
export const OrderDetailsMobileTableRowCellComponent: React.FC<IOrderDetailsTableRowProps> = (props: IOrderDetailsTableRowProps) => {
    ...
    return (
        <>
            ...
            <td>
                <div className={`${props.className}__product-info`}>
                    <span className={`${props.className}__product-name`}>
                        {productName}
                    </span>
            ...
            <td>
                <OrderDetailsExtraActionsPopupComponent
                    {...props}
                />
            </td>
        </>
...
export const OrderDetailsTableRowComponent: React.FC<IOrderDetailsTableRowProps> = (props: IOrderDetailsTableRowProps) => {
    ...
    const className = classnames(
        props.className, props.data.product.isProductAvailable ? `${props.className}__available-product` : `${props.className}__unavailable-product`);

    return (
        <tr className={className}>
            {
                isMobileView ? (
                    <OrderDetailsMobileTableRowCellComponent
                        {...props}
                    />
                ) : (
                    <OrderDetailsDesktopTableRowCellComponent
                        {...props}
                    />
                )
            }
