...
import { IBuyboxExcelImportViewProps } from '../../../modules/buybox/components/buybox-excel-import';
...
const BuyboxView: React.FC<IBuyboxViewProps & IBuyboxExtentionProps<IBuyboxData>> = props => {
    ...
    const { ModuleProps, MediaGalleryContainerProps, ProductInfoContainerProps,
        ...
        quantityLimitsMessages, max, shopSimilarDescription, unitOfMeasure, excelImport } = props;
    ...
    return (
        <Module {...ModuleProps}>
            ...
            <Node {...ProductInfoContainerProps}>
                ...
                { excelImport && _renderExcelImport(excelImport)}
                {unitOfMeasure} 
                {description}
...
const _renderExcelImport = (excelImport: IBuyboxExcelImportViewProps): JSX.Element | undefined  => {    
    const { ContainerProps, input } = excelImport;
    return (
        <Node {...ContainerProps}>
           {input}
        </Node>
...
const _renderShippingStatus = (findInStore: IBuyboxFindInStoreViewProps, actionContext: IActionContext, test2: () => void): JSX.Element => {
    return (
       <button className='ms-buybox__find-in-store-button' color="primary" 
             onClick={() => onClickHandler("44",actionContext)}>Check Shipping Status</button>
    );
};
...
// };
 ...
 const  onClickHandler = async (id: any, conts: IActionContext) =>{
     // Create an object of Input class and pass the data 
